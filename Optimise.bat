...
if %errorLevel% neq 0 (
    echo Failed to disable TCP heuristics.
) else (
    echo TCP heuristics successfully disabled.
)
echo.

:: Disable UDP offloading
echo Disabling UDP offloading...
netsh int udp set global uro=disabled
if %errorLevel% neq 0 (
    echo Failed to disable UDP offloading.
) else (
    echo UDP offloading successfully disabled.
)
echo.
